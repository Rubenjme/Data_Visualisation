...
data["Week"] = data["Timestamp"].dt.strftime("%Y-%U")              # Creo una nueva columna en el DataFrame que se llamará Week y que contendrá la fecha de la columna Timestamp
...
chart_def =  """
{
    ...
    subtitle: {
        text: 'Average ratings by week left for all courses'
    },
    xAxis: {
        reversed: false,
        title: {
...
def app():
    ...
    hc.options.tittle.text = "Average rating by Week"


    hc.options.xAxis.categories = list(week_average.index)
    hc.options.series[0].data = list(week_average.values)
